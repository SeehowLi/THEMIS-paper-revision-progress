...
- 每轮结束报一次离 8/25 的剩余天数
- 章节标题一律用短名词短语，不要冒号式标题，不要 "Part I / II" 式编号。总分关系放在段落开篇句里，不放在标题里。带冒号的标题看着像 AI 起的
- 写 Contributions / Abstract / Introduction 时，“厉害”必须来自可验证的洞察，不来自形容词。禁止 `novel` / `significant` / `first` / `state-of-the-art` 类词

### 给 Codex 写指令时必须包含

1. 多智能体拓扑：主 agent 编排且只有它写回执；调研与代码统计并行；编辑等依赖项；核验全程滚动。写主稿的权限只给一个 agent。
2. PDF 硬规则：读 PDF 前必须先调 `pdf2md` 转 MD，只读 MD，统一存 `论文MD/` 并配 manifest；开工先查目录，已有不重转。
3. 复用清单 + 不要做清单（不重查已结案、不做全库搜索、不重建词表、不跑 P3–P7 闸门）。
4. 定位表：`\label` + 当前行区间，不让它搜。
5. 授权用硬零集合制，不枚举允许类别：`numbers_with_units = 0`、图表 label 删除数 = 0；其余作为证据报告。
6. 冻结数字用带边界正则，并对核心数字做位置核对而非只数总量。

### 关于“含金量”

用户会要求把创新点包装得更有含金量。合法的手段只有四个：统一命名、陈述一般原理而非实例、显示克服的困难、量化。形容词加重（`novel` / `significant` / `substantially`）是反效果——审稿人已经质疑过 novelty，形容词只会坐实印象。含金量 = 可验证主张的密度。
...
给 Codex 写指令时必须考虑它的执行成本。每轮指令都要带：
复用清单（上轮产出、禁止重新生成）、不要做清单（不重查已结案事项、不做全库搜索、不重建词表）、定位表（\label + 行号，不让它搜）、外部文献读取的批量化、以及固定的计数口径。
我实测过：不给这些约束，它会抓取大量命令，浪费 token 和时间。
